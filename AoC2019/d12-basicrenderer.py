...
def render_board(coords, vels, size, num):
    ...
    #  key=lambda x: sum([abs(y) for y in x[1]]), reverse=True):
    # both = list(zip(coords, vels))
    # print(both)
    # print(sorted(both, key=lambda x: x[0][1]))
    # input(coords)
    # print(coords, vels)
    # coords, vels = zip(*sorted(zip(coords, vels),  key=lambda x: x[0][2]))
    # print(coords, vels)
    # input()
    # stuff = list(zip(coords, vels))
    # print(stuff)
    # stuff = sorted(stuff, key=lambda x: x[0][2])
    # stuff = zip(*sorted(zip(coords, vels)))
    # input(stuff)
    #print(coords, vels)
    ...
    for coord, vel in zip(coords, vels):
        ...
        if camera-10 - coord[2] < 0:
            continue
        x,y = coord[0]*2+mid[0], coord[1]*2+mid[1]
        # r = max(1, int(math.sqrt(abs(coord[2]) * 2)))
        # r = max(1, ((coord[2] + 20.0) / (x if (x := math.log(float(coord[2]))) > 0 else 1.0) * 2))
        r = max(1, 1/(camera-coord[2] if camera-coord[2] > 0 else 1) * 1000)
        total_vel = sum([abs(v) for v in vel]) / 40

        # highest = max(highest, total_vel)
        # lowest = min(lowest, total_vel)
