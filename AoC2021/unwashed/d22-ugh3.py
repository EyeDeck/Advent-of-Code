...
#
#
#         print(f'{existing} and {cuboid} overlap?{check_overlap(existing, cuboid)\
#         }, bb: {get_overlap(existing, cuboid)}')
...
def insert_cuboid(cuboids, cuboid, state):
    ...
    for ex, es in cuboids.items():
        if check_overlap(ex, cuboid):
            ...
            if overlap in cuboids:
                to_extend[overlap] = state - cuboids[overlap]
            else:
                to_extend[overlap] = state
    to_extend[cuboid] = state
    print('updated with', to_extend)
    cuboids.update(to_extend)


def get_ext_cuboid_area(coords, state):
    print('coords', coords)
    #return abs(coords[1][0] - coords[0][0]) * abs(coords[1][1] - coords[0][1]) * abs(coords[1][2] - coords[0][2]) * state
    #return abs(coords[0] - (coords[1] + 1)) * abs(coords[2] - (coords[3] + 1)) * abs(coords[4] - (coords[5] + 1)) * state
    return (coords[1][0] - coords[0][0] + 1) * (coords[1][1] - coords[0][1] + 1) * (coords[1][2] - coords[0][2] + 1) * state
...
def p2():
    ...
    for line in data:
        ...
        # insert_cuboid(ranges, coords, state)
        # ranges.add(coords)
        insert_cuboid(ranges, coords, state)
        ...
        for k,v in ranges.items():
            # print('asd', k,v)
            cum += get_ext_cuboid_area(k,v)
        print('after step:', cum)
